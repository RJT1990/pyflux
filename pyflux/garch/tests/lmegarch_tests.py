...
jpm = DataReader('JPM',  'yahoo', datetime(2011,1,1), datetime(2016,3,10))
...
def a_test_no_terms():
    model = pf.LMEGARCH(data=data, p=0, q=0)
    x = model.fit()
...
def a_test_couple_terms():
    model = pf.LMEGARCH(data=data, p=1, q=1)
    x = model.fit()
...
def a_test_predict_length():
    model = pf.LMEGARCH(data=data, p=2, q=2)
    x = model.fit()
    x.summary()
...
def a_test_predict_is_nans():
    model = pf.LMEGARCH(data=data, q=2, p=2)
    x = model.fit()
    x.summary()
...
def a_test_predict_intervals_bbvi():
    model = pf.LMEGARCH(data=data, q=1, p=1)
...
def a_test_lev_bbvi_mini_batch():
    model = pf.LMEGARCH(data=data, p=1, q=1)
    model.add_leverage()
...
def a_test_lev_bbvi_elbo():
    model = pf.LMEGARCH(data=data, p=1, q=1)
    model.add_leverage()
    x = model.fit('BBVI',iterations=300, map_start=False, record_elbo=True)
    assert(x.elbo_records[-1]>x.elbo_records[0])

def a_test_lev_bbvi_mini_batch_elbo():
    model = pf.LMEGARCH(data=data, p=1, q=1)
    model.add_leverage()
    x = model.fit('BBVI',iterations=300, map_start=False, mini_batch=32, record_elbo=True)
    assert(x.elbo_records[-1]>x.elbo_records[0])
...
def a_test_lev_predict_is_length():
    model = pf.LMEGARCH(data=data, p=2, q=2)
    model.add_leverage()
    x = model.fit()
    assert(model.predict_is(h=5).shape[0] == 5)

def a_test_lev_predict_nonconstant():
    model = pf.LMEGARCH(data=data, p=2, q=2)
    model.add_leverage()
    x = model.fit()
    predictions = model.predict(h=10, intervals=False)
...
def a_test_lev_predict_is_nonconstant():
    model = pf.LMEGARCH(data=data, p=2, q=2)
    model.add_leverage()
    x = model.fit()
    predictions = model.predict_is(h=5, intervals=False)
...
def a_test_lev_predict_is_nans():
    model = pf.LMEGARCH(data=data, q=2, p=2)
    model.add_leverage()
    x = model.fit()
    x.summary()
...
def a_test_lev_predict_intervals():
    model = pf.LMEGARCH(data=data, q=2, p=2)
    model.add_leverage()
    x = model.fit()
    predictions = model.predict(h=10, intervals=True)
...
def a_test_lev_predict_is_intervals():
    model = pf.LMEGARCH(data=data, q=2, p=2)
    model.add_leverage()
    x = model.fit()
    predictions = model.predict_is(h=10, intervals=True)
...
def a_test_lev_predict_intervals_bbvi():
    model = pf.LMEGARCH(data=data, q=1, p=1)
    model.add_leverage()
    x = model.fit('BBVI', iterations=100)
    predictions = model.predict(h=10, intervals=True)
...
def a_test_lev_predict_is_intervals_bbvi():
    model = pf.LMEGARCH(data=data, q=1, p=1)
    model.add_leverage()
    x = model.fit('BBVI', iterations=100)
    predictions = model.predict_is(h=10, intervals=True)
...
def a_test_lev_predict_intervals_mh():
    model = pf.LMEGARCH(data=data, q=1, p=1)
    model.add_leverage()
    x = model.fit('M-H', nsims=400)
    predictions = model.predict(h=10, intervals=True)
...
def a_test_lev_predict_is_intervals_mh():
    model = pf.LMEGARCH(data=data, q=1, p=1)
    model.add_leverage()
    x = model.fit('M-H', nsims=400)
    predictions = model.predict_is(h=10, intervals=True)
