...
def test_no_terms():
    model = pf.SEGARCH(data=data, p=0, q=0)
    x = model.fit()
...
def test_couple_terms():
    model = pf.SEGARCH(data=data, p=1, q=1)
    x = model.fit()
...
def test_predict_length():
    model = pf.SEGARCH(data=data, p=2, q=2)
    x = model.fit()
    x.summary()
...
def test_predict_is_nans():
    model = pf.SEGARCH(data=data, q=2, p=2)
    x = model.fit()
    x.summary()
...
def test_predict_intervals_bbvi():
    model = pf.SEGARCH(data=data, q=1, p=1)
