...
def test_no_terms():
    ...
    """
    model = pf.ARIMAX(formula="y ~ x1", data=data, ar=0, ma=0, family=pf.Laplace())
    x = model.fit()
...
def test_couple_terms():
    ...
    """
    model = pf.ARIMAX(formula="y ~ x1", data=data, ar=1, ma=1, family=pf.Laplace())
    x = model.fit()
...
def test_couple_terms_integ():
    ...
    values are not nan
    """
    model = pf.ARIMAX(formula="y ~ x1", data=data, ar=1, ma=1, integ=1, family=pf.Laplace())
    x = model.fit()
    assert(len(model.latent_variables.z_list) == 5)
    lvs = np.array([i.value for i in model.latent_variables.z_list])
    assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_predict_length():
    ...
    """
    model = pf.ARIMAX(formula="y ~ x1", data=data, ar=2, ma=2, family=pf.Laplace())
    x = model.fit()
    x.summary()
...
def test2_no_terms():
    ...
    are not nan
    """
    model = pf.ARIMAX(formula="y ~ x1 + x2", data=data, ar=0, ma=0, family=pf.Laplace())
    x = model.fit()
...
def test2_couple_terms():
    ...
    are not nan
    """
    model = pf.ARIMAX(formula="y ~ x1 + x2", data=data, ar=1, ma=1, family=pf.Laplace())
    x = model.fit()
...
def test2_predict_length():
    ...
    Tests that the length of the predict dataframe is equal to no of steps h
    """
    model = pf.ARIMAX(formula="y ~ x1 + x2", data=data, ar=2, ma=2, family=pf.Laplace())
    x = model.fit()
    x.summary()
    assert(model.predict(h=5, oos_data=data_oos).shape[0] == 5)
