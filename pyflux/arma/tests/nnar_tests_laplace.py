...
def test_no_terms():
    ...
    """
    model = pf.ARIMA(data=data, ar=0, ma=0, family=pf.Laplace())
    x = model.fit()
...
def test_couple_terms():
    ...
    latent variables are not nan
    """
    model = pf.ARIMA(data=data, ar=1, ma=1, family=pf.Laplace())
    x = model.fit()
...
def test_couple_terms_integ():
    ...
    latent variables are not nan
    """
    model = pf.ARIMA(data=data, ar=1, ma=1, integ=1, family=pf.Laplace())
    x = model.fit()
    assert(len(model.latent_variables.z_list) == 4)
    lvs = np.array([i.value for i in model.latent_variables.z_list])
    assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_predict_length():
    ...
    """
    model = pf.ARIMA(data=data, ar=2, ma=2, family=pf.Laplace())
    x = model.fit()
