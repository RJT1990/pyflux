...
def test_bbvi_elbo():
    ...
    """
    model = ARIMAX(formula="y ~ x1", data=data, ar=1, ma=1, family=Poisson())
    x = model.fit('BBVI', iterations=300, record_elbo=True, map_start=False, quiet_progress=True)
...
def test_bbvi_mini_batch_elbo():
    ...
    """
    model = ARIMAX(formula="y ~ x1", data=data, ar=1, ma=1, family=Poisson())
    x = model.fit('BBVI',iterations=300, mini_batch=32, record_elbo=True, map_start=False, quiet_progress=True)
    assert(x.elbo_records[-1]>x.elbo_records[0])
