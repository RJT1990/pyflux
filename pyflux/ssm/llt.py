...
class LLT(tsm.TSM):
	...
	**** LOCAL LINEAR TREND MODEL ****
	...
	def __init__(self,data,integ=0,target=None):
		...
		self.param_desc.append({'name' : 'Sigma_lev','index': 1, 'prior': ifr.Uniform(transform='exp'), 'q': dst.Normal(0,3)})
		self.param_desc.append({'name' : 'Sigma_trd','index': 2, 'prior': ifr.Uniform(transform='exp'), 'q': dst.Normal(0,3)})
	...
	def model(self,beta):
		...
		Q[0][0] = self.param_desc[1]['prior'].transform(beta[1])
		Q[1][1] = self.param_desc[2]['prior'].transform(beta[2])
