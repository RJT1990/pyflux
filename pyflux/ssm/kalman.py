...
def univariate_KFS(y,Z,H,T,Q,R,mu):
	...
	# BACKWARDS (SMOOTHING)
	for t in reversed(xrange(y.shape[0])):
		if t != 0:
			L[:,:,t] = T - np.dot(K[:,t],Z)
...
def univariate_kalman_fcst(y,Z,H,T,Q,R,mu,h):
	...
	for t in xrange(0,y.shape[0]+h):
		if t >= y.shape[0]:
			...
			v[t] = y[t] - np.dot(Z,a[:,t]) - mu
			F[:,:,t] = np.dot(np.dot(Z,P[:,:,t]),Z.T) + H.ravel()[0]
			K = np.dot(np.dot(T,P[:,:,t]),Z.T)/(F[:,:,t]).ravel()[0]

		a[:,t+1] = np.dot(T,a[:,t]) + np.dot(K,v[t]) 

		P[:,:,t+1] = np.dot(np.dot(T,P[:,:,t]),T.T) + np.dot(np.dot(R,Q),R.T) - F[:,:,t].ravel()[0]*np.dot(np.array([K]).T,np.array([K]))
