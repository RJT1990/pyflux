...
class GASNormal(GAS):
    ...
    **** NORMAL GENERALIZED AUTOREGRESSIVE SCORE (GAS) MODELS ****
    ...
    def __init__(self,data,ar,sc,integ=0,target=None,gradient_only=False):
        ...
        super(GASNormal,self).__init__(data=data,ar=ar,sc=sc,integ=integ,
            target=target,gradient_only=gradient_only)

        self.model_name = "NORMAL GAS(" + str(self.ar) + "," + str(self.integ) + "," + str(self.sc) + ") REGRESSION"
        self.dist = 'Normal'
        self.link = np.array
        self.scale = True
        self.shape = False
    ...
    def adj_score_function(self,y,mean,scale,shape,skewness):
        return Normal1Score.mu_adj_score(y, mean, np.power(scale,2))
    ...
    def neg_loglik(self,beta):
        ...
        return -np.sum(ss.norm.logpdf(Y,loc=theta,scale=self.parameters.parameter_list[-1].prior.transform(beta[-1])))  

    def default_score_function(self,y,mean,scale,shape,skewness):
        return Normal1Score.mu_score(y, mean, np.power(scale,2))
    ...
    def predict_is(self,h=5):
        ...
        for t in range(0,h):
            x = GASNormal(ar=self.ar,sc=self.sc,integ=self.integ,data=self.data_original[:-h+t])
            x.fit(printer=False)
