...
def test_exponential_no_terms():
	...
	"""
	model = pf.GASX(formula="y ~ x1", data=data, ar=0, sc=0, family=pf.GASExponential())
	x = model.fit()
...
def test_exponential_couple_terms():
	...
	"""
	model = pf.GASX(formula="y ~ x1", data=data, ar=1, sc=1, family=pf.GASExponential())
	x = model.fit()
...
def test2_exponential_no_terms():
	...
	are not nan
	"""
	model = pf.GASX(formula="y ~ x1 + x2", data=data, ar=0, sc=0, family=pf.GASExponential())
	x = model.fit()
...
def test2_exponential_couple_terms():
	...
	are not nan
	"""
	model = pf.GASX(formula="y ~ x1 + x2", data=data, ar=1, sc=1, family=pf.GASExponential())
	x = model.fit()
