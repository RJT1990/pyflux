...
def test_laplace_couple_terms():
	...
	"""
	model = pf.GASLLEV(data=data, family=pf.Laplace())
	x = model.fit()
...
def test_laplace_couple_terms_integ():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, integ=1, family=pf.Laplace())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 2)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_laplace_bbvi_mini_batch_elbo():
    ...
    """
    model = pf.GASLLEV(data=data, family=pf.Laplace())
    x = model.fit('BBVI',iterations=100, mini_batch=32, record_elbo=True, map_start=Fa)
    assert(x.elbo_records[-1]>x.elbo_records[0])
