...
def test_couple_terms():
	...
	"""
	model = pf.GASLLEV(data=data, family=pf.GASNormal())
	x = model.fit()
...
def test_couple_terms_integ():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, integ=1, family=pf.GASNormal())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 2)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_t_couple_terms():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, family=pf.GASt())
	x = model.fit()
...
def test_t_couple_terms_integ():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, integ=1, family=pf.GASt())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 3)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_skewt_couple_terms():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, family=pf.GASSkewt())
	x = model.fit()
...
def test_skewt_couple_terms_integ():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, integ=1, family=pf.GASSkewt())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 4)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)
...
def test_laplace_couple_terms_integ():
	...
	latent variables are not nan
	"""
	model = pf.GASLLEV(data=data, integ=1, family=pf.GASLaplace())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 2)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)
