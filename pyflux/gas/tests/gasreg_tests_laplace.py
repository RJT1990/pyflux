...
def test_laplace_no_terms():
	...
	"""
	model = pf.GASReg(formula="y ~ x1", data=data, family=pf.GASLaplace())
	x = model.fit()
...
def test2_laplace_no_terms():
	...
	are not nan
	"""
	model = pf.GASReg(formula="y ~ x1 + x2", data=data, family=pf.GASLaplace())
	x = model.fit()
	assert(len(model.latent_variables.z_list) == 3)
	lvs = np.array([i.value for i in model.latent_variables.z_list])
	assert(len(lvs[np.isnan(lvs)]) == 0)

def test2_laplace_bbvi():
	"""
...
def test2_laplace_predict_length():
	"""
	Tests that the length of the predict dataframe is equal to no of steps h
	"""
	model = pf.GASReg(formula="y ~ x1 + x2", data=data, family=pf.GASLaplace())
...
def test2_laplace_predict_nans():
	"""
	Tests that the predictions are not NaNs
	"""
	model = pf.GASReg(formula="y ~ x1 + x2", data=data, family=pf.GASLaplace())
...
def test2_laplace_predict_is_nans():
	"""
	Tests that the predictions in-sample are not NaNs
	"""
	model = pf.GASReg(formula="y ~ x1 + x2", data=data, family=pf.GASLaplace())
