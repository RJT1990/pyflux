...
noise = np.random.normal(0,0.07,200)
data = np.zeros(200)

for i in range(1,len(data)):
    data[i] = 0.9*data[i-1] + noise[i]

countdata = np.random.poisson(np.exp(data),200)
...
def test_poisson_no_terms():
	...
	"""
	model = pf.GAS(data=countdata, ar=0, sc=0, family=pf.Poisson())
	x = model.fit()
...
def test_poisson_predict_nonconstant():
    """
